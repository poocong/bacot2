    ...
    async def start(self):
        ...
        if FORCE_SUB_CHANNEL:
            try:
                ...
                self.LOGGER(__name__).info(
                    "\nBot Berhenti. Silahkan bertanya ke @DessXtrash untuk Bantuan"
                )
                sys.exit()
        if FORCE_SUB_CHANNELL:
            try:
                link = await self.export_chat_invite_link(FORCE_SUB_CHANNELL)
                self.invitelink = link
            except Exception as a:
                self.LOGGER(__name__).warning(a)
                self.LOGGER(__name__).warning(
                    "Bot tidak dapat Mengambil link Undangan dari FORCE_SUB_CHANNELL!"
                )
                self.LOGGER(__name__).warning(
                    f"Silakan periksa kembali var FORCE_SUB_CHANNELL dan Pastikan Bot anda Admin di Channel dengan izin link invite Pengguna melalui link undangan, Subs Channel Saat Ini: {FORCE_SUB_CHANNELL}"
                )
                self.LOGGER(__name__).info(
                    "\nBot Berhenti. Silahkan bertanya ke @DessXtrash untuk Bantuan"
                )
        ...
        if FORCE_SUB_GROUP:
            try:
                ...
                self.LOGGER(__name__).info(
                    "\nBot Berhenti. silakan hubungi https://t.me/DessXtrash untuk Bantuan"
                )
                sys.exit()
        try:
            ...
            self.LOGGER(__name__).info(
                "\nBot Berhenti. Silahkan Bertanya ke https://t.me/DessXtrash untuk Bantuan"
            )
            sys.exit()

        self.set_parse_mode("html")
        self.LOGGER(__name__).info(
            f"[🔥 BOT FILE SHARING BERHASIL DIAKTIFKAN! 🔥]\n\nBOT Dibuat oleh @{OWNER}\nJika @{OWNER} Membutuhkan Bantuan, Silahkan Tanyakan di Grup https://t.me/Bukan_guudlooking"
        )
